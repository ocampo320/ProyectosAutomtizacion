...
public class BuscarVuelo implements Task {
	...
	public <T extends Actor> void performAs(T actor) {
		...
		actor.attemptsTo(Click.on(InicioDespegar.VUELOS), Enter.theValue(origen).into(InicioDespegar.ORIGEN),
				Hit.the(Keys.ENTER).into(InicioDespegar.ORIGEN), Enter.theValue(destino).into(InicioDespegar.DESTINO),
				Hit.the(Keys.ENTER).into(InicioDespegar.DESTINO), Click.on(InicioDespegar.CALENDARIO),
